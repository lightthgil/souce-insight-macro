/******************************************************************************
  ...
  9.日    期   : 2011年3月18日
    作    者   : 蒋博 jiangbo
    修改内容   : 修改函数、头文件、修改日志的注释符合烽火标准
                 对函数的每个参数添加注释
                 将大部分地方修改为支持无限长字符串、字符串自动分行

 10.日    期   : 2011年3月22日
    作    者   : 蒋博 jiangbo
    修改内容   : 自动扩展码分别为"ab"、"db"、"mb"、"as"、"ds"、"ms"的注释原因提示用户输入
                 自动扩展码分别为"ab"、"db"、"mb"的注释自动分行
                 文件头的注释内容前后添加空行
                 
 11.日    期   : 2011年6月1日
    作    者   : 蒋博 jiangbo
    修改内容   : 自动扩展码分别为"ab"、"db"、"mb"的注释前缀改短
 ...
 13.日    期   : 2011年6月20日
    作    者   : 蒋博
    修改内容   : 自动扩展码分别为"abg"、"dbg"、"mbg"注释原因提示用户输入

 14.日    期   : 2011年9月9日
    作    者   : 蒋博
    修改内容   : #后添加分号，使source insight排版正常
    
 15. 日期:2012年6月5日
     作者:牟晓弦xxmou
     修改内容:
     (1) 根据产品开发平台管理部新发布的软件实现规范，修改完善代码添加注释的快捷工具quicker.em
     (2) 头文件注释中，添加“版本号：”
     (3) 代码行注释中，添加“Mantis号：”

 16.日    期   : 2012年7月5日
    作    者   : 蒋博
    修改内容   : 增加是否弹出对话框提示输入的判断
                 增加当弹出对话框提示输入时，每个判断、循环分支添加注释
                 修改自动扩展码为"ap"的注释
******************************************************************************/
...
macro AutoExpand()
{
    ...
    /* 是否需要弹出对话框提示输入 */
    askForInput = getreg(NEEDASKFORINPUT)
    if(askForInput != 0)
    {
        askForInput = 1
    }
    ...
    if(language == 1)
    {
        ExpandProcEN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel,askForInput)
    }
    else
    {
        ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel,askForInput)
    }
}
...
*****************************************************************************/
macro ExpandProcEN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel,askForInput)
{
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "ab")
    {
        ...
        {
            InsBufLine(hbuf, ln, "@szLine1@/* add begin by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, reason: */");        
        }
        return
    }
    else if (szCmd == "ae")
    ...
    else if (szCmd == "db")
    {
        ...
        szQuestion = GetReg ("PNO")
            if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, reason: */");
        }
    ...
    else if (szCmd == "mb")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, reason: */");
        }
        return
    }
    else if (szCmd == "me")
    ...
    else if (szCmd == "as")
    {
        ...
        if(strlen(szQuestion)>0)
        { 
        	InsBufLine(hbuf, ln, "@szLine1@/* add by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* add by @szMyName@, @sz@@szMonth@@szDay@, reason: */");        
        }
        return
    }
    else if (szCmd == "ds")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete by @szMyName@, @sz@@szMonth@@szDay@, reason: */");        
        }
        return
    }
    else if (szCmd == "ms")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify by @szMyName@, @sz@@szMonth@@szDay@,   PN:@szQuestion@ reason: */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify by @szMyName@, @sz@@szMonth@@szDay@, reason: */");        
        }
        return
    }
    else
...
*****************************************************************************/
macro ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel,askForInput)
{
    szCmd = wordinfo.szWord
    hwnd = GetCurrentWnd()
    if (hwnd == 0)
    ...
    else if (szCmd == "while" || szCmd == "wh")
    {
        ...
        {
            szContent = Ask("请输入循环的条件")
            SetBufSelText(hbuf, " ( @szContent@ ) /* 当@szContent@时执行循环 */")
        }
    ...
    else if (szCmd == "if")
    {
        ...
        {
            szContent = Ask("请输入判断的条件")
            SetBufSelText(hbuf, " ( @szContent@ ) /* 当@szContent@成立时执行 */")
        }
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}");
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "ife")
    {
        ...
        {
            szContent = Ask("请输入判断的条件")
            PutBufLine(hbuf, ln, szLine1 # "if ( @szContent@ ) /* 当@szContent@成立时执行 */")
        }
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}");
        ...
        {
            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else /* 当@szContent@不成立时执行 */");
        }
        InsBufLine(hbuf, ln + 5, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 6, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 7, "@szLine1@" # "}");
    ...
    else if (szCmd == "ifs")
    {
        ...
        {
            szContent1 = Ask("请输入判断的条件1")
            PutBufLine(hbuf, ln, szLine1 # "if ( @szContent1@ ) /* 当@szContent1@成立时执行 */")
        }
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}");
        ...
        {
            szContent2 = Ask("请输入当判断条件1不成立时的判断条件2")
            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else if ( @szContent2@ ) /* 当@szContent1@不成立,@szContent2@成立时执行 */");
        }
        InsBufLine(hbuf, ln + 5, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 6, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 7, "@szLine1@" # "}");
        ...
        {
            InsBufLine(hbuf, ln + 8, "@szLine1@" # "else /* 当@szContent1@、@szContent2@都不成立时执行 */");
        }
        InsBufLine(hbuf, ln + 9, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 10, "@szLine@" # "#;");
        InsBufLine(hbuf, ln + 11, "@szLine1@" # "}");
    ...
    else if (szCmd == "for")
    {
        ...
        newsel.ichLim = GetBufLineLength (hbuf, ln)
        SetWndSel(hwnd, newsel)
        SetBufSelText(hbuf, " (@szVar@ = #; @szVar@ #; @szVar@++) /* 对变量@szVar@进行循环 */")
    }
    else if (szCmd == "fo")
    {
        SetBufSelText(hbuf, "r (ulI = 0; ulI < #; ulI++) /* 对变量ulI进行循环 */")
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{")
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;")
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}")
        symname =GetCurSymbol ()
    ...
    else if (szCmd == "switch" || szCmd == "sw")
    {
        ...
        {
            szContent = Ask("请输入判断的条件")
            SetBufSelText(hbuf, " ( @szContent@ ) /* 判断匹配条件@szContent@ */")
        }
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{")
        InsertMultiCaseProc(hbuf,szLine1,nSwitch)
    }
    else if (szCmd == "do")
    {
        ...
        {
            szContent = Ask("请输入判断的条件")
            InsBufLine(hbuf, ln + 3, "@szLine1@" # "} while ( @szContent@ ); /* 循环直到@szContent@不成立 */")
        }
    }
    else if (szCmd == "case" || szCmd == "ca" )
    {
        ...
        {
            szContent = Ask("请输入匹配的条件")
            SetBufSelText(hbuf, " @szContent@ : /* 匹配@szContent@ */")
        }
        InsBufLine(hbuf, ln + 1, "@szLine@" # "#;")
        InsBufLine(hbuf, ln + 2, "@szLine@" # "break;")
    }
    ...
    {   
        ...
        {
            szContent = Ask("请输入增加的原因")
        }
        szContent = cat("add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:", szContent)
        CommentContent(hbuf,ln,szLeft,szContent,1)
...
//        ln = CommentContent(hbuf,ln + 1,szLeft,szContent,1)
    ...
    else if (szCmd == "ab")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            ...
            //szLeft = "@szLine1@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:"
            szContent = cat("add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:", szContent)
        }
        else
        {
            //InsBufLine(hbuf, ln, "@szLine1@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, 原因: */");
            //szLeft = "@szLine1@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, 原因:"
            szContent = cat("add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:", szContent)
    ...
    else if (szCmd == "db")
    {
        ...
        {
            szContent = Ask("请输入删除的原因")
        }
        szLeft = "@szLine1@/* "
        if(strlen(szQuestion) > 0)
        {
            //InsBufLine(hbuf, ln, "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因: */");
            //szLeft = "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:"
            szContent = cat("delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:", szContent)
        }
        else
        {
            //InsBufLine(hbuf, ln, "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, 原因: */");
            //szLeft = "@szLine1@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, 原因:"
            szContent = cat("delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:", szContent)
        }
        CommentContent(hbuf,ln,szLeft,szContent,1)
    ...
    else if (szCmd == "mb")
    {
        ...
        {
            szContent = Ask("请输入修改的原因")
        }
        szLeft = "@szLine1@/* "
        if(strlen(szQuestion) > 0)
        {
            //InsBufLine(hbuf, ln, "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因: */");
            //szLeft = "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:"
            szContent = cat("modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@ 原因:", szContent)
        }
        else
        {
            //InsBufLine(hbuf, ln, "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, 原因: */");
            //szLeft = "@szLine1@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, 原因:"
            szContent = cat("modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:", szContent)
        }
        CommentContent(hbuf,ln,szLeft,szContent,1)
    ...
    else if (szCmd == "as")
    {
        ...
        if(strlen(szQuestion)>0)
        { 
        	InsBufLine(hbuf, ln, "@szLine1@/* add by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* add by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */");        
        }
        return
    }
    else if (szCmd == "ds")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* delete by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */");        
        }
        return
    }
    else if (szCmd == "ms")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* modify by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */");        
        }
        return
    }
    else
...
/*****************************************************************************
 ...
 功能描述  : 块命令处理函数，根据所选区域上一行的自动扩展码自动完成基本语句
 输出参数  : 无
 返 回 值  : 
 调用函数  : 
 被调函数  : 
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/***********************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
 //   InsBufLine(hbuf, ln + 2,  " ************** 版权所有 (C), @szYear@,烽火通信科技股份有限公司宽带产品部 **************")
 //   InsBufLine(hbuf, ln + 3,  "")
 //   InsBufLine(hbuf, ln + 4,  " ***********************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
    InsBufLine(hbuf, ln + 2,  " * 文 件 名   : @sz@")
    /*InsBufLine(hbuf, ln + 6,  "  版 本 号   : 初稿")*/
    InsBufLine(hbuf, ln + 3,  " * 负 责 人   : @szName@")
    InsBufLine(hbuf, ln + 4,  " * 创建日期   : @szYear@年@szMonth@月@szDay@日")
    InsBufLine(hbuf, ln + 5  " * 版 本 号   : ")
...
//    InsBufLine(hbuf, ln + 18, "")
    ...
    //设置光标位置为注释的最后一行
    SetBufIns(hbuf,nlnDesc + 11,0)
    
    //将原"文件描述"行删掉
    DelBufLine(hbuf,nlnDesc +6)
    
    //自动排列显示功能描述
    CommentContent(hbuf,nlnDesc+6," * 文件描述   : ",szContent,0)
}
...
macro CommentContent1 (hbuf,ln,szPreStr,szContent,isEnd)
{
    ...
    {
        i = 0
        while  (iLen - i > 100 - k ) /* modify begin by 蒋博,jiangbo, 20110620, 原因:每行100个字符 */
        {
            j = 0
            while(j < 100 - k)   /* modify begin by 蒋博,jiangbo, 20110620, 原因:每行100个字符 */
            {
                iNum = szContent[i + j]
                //如果是中文必须成对处理
...
macro CommentContent (hbuf,ln,szPreStr,szContent,isEnd)
{
    ...
    while ( nIdx < lnMax) 
    {
        ...
        else
        {
            i = 0
            //以每行75个字符处理
            while  (iLen - i > 118 - k ) /* modify begin by 蒋博,jiangbo, 20110620, 原因:每行120个字符 */
            {
                j = 0
                while(j < 118 - k)   /* modify begin by 蒋博,jiangbo, 20110620, 原因:每行120个字符 */
                {
                    iNum = szContent[i + j]
                    if( AsciiFromChar (iNum)  > 160 )
                    {
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if(newFunc != 1)
    {
        ...
        if(strlen(symbol) > 0)
        {
            ...
            //szRet =  GetFirstWord(szTemp)   /* 不用获取第一个字符，szTemp已经是函数返回值了 */
            szRet = szTemp
            if(symbol.Type == "Method")
            {
    ...
    if(newFunc != 1)
    {
        ...
        while ( i < lnMax) 
        {
            ...
            if(szParamDescribe == "#")    //输入"#"表示切换到输出参数
            {
                break;
            }
        ...
        //对于已经存在的函数插入函数输出参数
        iIns = 0    //初试为0
        ln = ln - 1    //后面插入的时候为了和新函数统一，用ln+5，所以这里需要将ln-1
        szIns = " * 输出参数  : "
        //i = 0    /* i延续输入参数后面 */
        while ( i < lnMax) 
        {
            ...
            //插入参数描述
            szParamAsk = cat("请输出参数描述: ",szTmp)
            szParamDescribe = Ask(szParamAsk)
            szTmp = cat(szTmp,szParamDescribe)
            
            ln = ln + 1		//这句是关键，每找到一个参数将ln加1，保证今后在ln+?行都是在所有参数基础后加的
            szTmp = cat(szIns,szTmp)
            InsBufLine(hbuf, ln+5, "@szTmp@")	//在第ln+5行插入是因为前面每个参数都ln=ln+1，其实是从ln5行开始每个参数插入一行
            iIns = 1	//函数是否存在参数标记
            szIns = "               "		//第二个参数前面留空格对齐
            i = i + 1
        }
        ...
        {
            ln = ln + 1		//为了保持原有参数的一致
            InsBufLine(hbuf, ln+5, " * 输出参数  : 无")//在第ln+5行插入是因为前面每个参数都ln=ln+1，其实是从ln6行开始每个参数插入一行
        }
    ...
    nLen = strlen(szRet);
    szBlank = CreateBlankString(nMaxParamSize - nLen + 2)
    szRet = cat(szRet,szBlank)

    //插入返回值描述
    szParamAsk = cat("请返回值描述: ",szRet)
    szParamDescribe = Ask(szParamAsk)
    szRet = cat(szRet,szParamDescribe)
    szRet = cat(szIns,szRet)
    InsBufLine(hbuf, ln+6, "@szRet@")
...
macro ConfigureSystem()
{
    ...
    askForInput = ASK("Do you want pop-up msgbox to input: 0 yes , 1 no");
    if(askForInput == "#")
    {
       SetReg ("NEEDASKFORINPUT", "0")
    }
    else
    {
       SetReg ("NEEDASKFORINPUT", askForInput)
    }
}
...
macro ReviseCommentProc(hbuf,ln,szCmd,szMyName,szLine1)
{
    ...
    else if (szCmd == "ab")
    {
        ...
        {
            InsBufLine(hbuf, ln, "@szLine1@/* add begin by @szMyName@, Mantis号:无, @sz@@szMonth@@szDay@ */");        
        }
        return
    }
    else if (szCmd == "ae")
...
macro InsertReviseAdd()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
    }
    else
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* add begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */";
    }
...
macro InsertReviseDel()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
    }
    else
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* delete begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */");     
    }
...
macro InsertReviseMod()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:@szQuestion@, 原因:@szContent@ */");
    }
    else
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* modify begin by @szMyName@, @sz@@szMonth@@szDay@, Mantis号:无, 原因:@szContent@ */");   
    }
